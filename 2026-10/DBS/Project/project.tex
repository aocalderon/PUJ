...
A research group is collecting gameplay telemetry from a \href{https://github.com/aocalderon/trajectory_doom}{\emph{hacked} version}\footnote{The source code is available at \url{https://github.com/aocalderon/trajectory_doom}} of \href{https://www.chocolate-doom.org/wiki/index.php/Chocolate_Doom}{\texttt{chocolate-doom}}. This build emits per-tic data (on screen and/or to file) that includes the player position $(x,y,z)$, facing \emph{angle}, \emph{momentum} vector, \emph{point-of-view} (FOV/camera), and combat stats (health, armor, ammo). It also logs meta-information such as \emph{tic} number, \emph{episode}, \emph{map}, and \emph{sector}.

The team aims to aggregate data across multiple play sessions to identify movement trends and behavioral patterns. This analysis will facilitate the detection of player cooperation or potential sabotage. Student volunteers provide demographics (age, gender, experience) and complete \emph{one} of these UX instruments:
...
Produce an ER diagram capturing main relationships.  The following entities and relationships are provided as examples; you are encouraged to propose your own.
...
Implement SQL solutions for \textbf{any six (6) of the eight (8)} proposed analytical queries. \textit{Note: The provided hints utilize generic table and attribute names; you are required to adapt these to the specific nomenclature defined in your database schema.}
...
\subsection*{Part A: Conceptual and Logical Design (Week 1--2)}
\begin{enumerate}[leftmargin=*]
  \item Write assumptions and requirements (functional/non-functional, ethics).
  \item Produce an initial draft of ER diagram with cardinalities and key attributes.
  \item Derive relational schema; list all FKs, PKs, and constraints (Data Dictionary).
\end{enumerate}

\subsection*{Part B: Implementation \& Ingestion (Week 3--4)}
\begin{enumerate}[leftmargin=*]
  \item Implement DDL in your DBMS (PostgreSQL recommended).
  \item Create scripts and \textit{optional} staging tables to load sample telemetry logs (TSV).
  \item Populate the \textit{UX Instrument} tables with the survey of your choice (PENS,GUESS or BANGS).
  \item Insert synthetic sample data (at least 3 different episodes, 6+ players, $\geq$ 20k telemetry rows).
\end{enumerate}

\subsection*{Part C: Queries, Indexing, and Reporting (Week 5--6)}
\begin{enumerate}[leftmargin=*]
  \item Implement at least 6 analytical queries (see Section \ref{sec:queries}).
  ...
  \item Provide and document a simple mechanism (could be a \href{https://en.wikipedia.org/wiki/Make_(software)}{\texttt{Makefile}} or shell script) to recreate the schema and load samples.
\end{enumerate}

\section{Submission Format}

Submit a single \textbf{\large PDF} report with:
\begin{itemize}[leftmargin=*]
  \item \textbf{ER diagram}, \textbf{relational schema}, and \textbf{rationale} (brief explanation about your model and schema).
  \item \textbf{DDL/constraints} (appendix with code snippets).
  \item \textbf{ETL description} + sample of raw telemetry.
  \item \textbf{Queries + results} (screenshots/tables).
  \item \textbf{Index evaluation}.
...
Analytics queries (6+) correctness \& insight & 15 \\
...
\section{Bonus}
\begin{itemize}[leftmargin=*]
  \item If available, enable extensions like \texttt{citext}, \texttt{uuid-ossp} or \texttt{pgcrypto} for authorization or \texttt{postgis} for spatial indexing and analysis.
\end{itemize}

\href{https://youtu.be/F7GM4aJ39Ks?si=H3JMy9Td1j-eG4kL}{\textsc{RIP \& TEAR}}
