...
\title[SQL Functions]{Databases}
\subtitle{SQL Functions.}
...
\begin{frame}{Databases: SQL Functions.}
    \centering
  ...
  BEGIN
    FOR rw IN SELECT * FROM posts WHERE pk = p_id LOOP
      ...
      ret.record_data := hstore(
        ARRAY['title', rw.title, 'Title and Content', format('%s %s', rw.title, rw.content)]
      );
      RETURN NEXT ret;
    END LOOP;
    RETURN;
...
\begin{frame}{Additional Resources}
    \begin{enumerate}
        \item \textit{``PostgreSQL Documentation - User-Defined Functions''} by The PostgreSQL Global Development Group.  Available at \url{https://www.postgresql.org/docs/current/xfunc.html}.
        \item \textit{``How to write SQL Functions in PostgreSQL FAST | Full Tutorial | Step-by-Step''} by Software Nuggets.  Available at \url{https://www.youtube.com/watch?v=GNTap3iVg3c}.
        \item \textit{``Como crear una función (Stored Procedure) - FUNCTION | PostgreSQL \#31''} by TecnoBinaria.  Available at \url{https://www.youtube.com/watch?v=cQPcqIkBV-U}.
    \end{enumerate}
\end{frame}
