...
\subtitle{Lecture 10: Database Security.}
\author{Mullins, 2013.}
\date{\today}
...
\begin{frame}{Database Administration: Database Security.}
    \centering
    \includegraphics[width=0.325\textwidth]{figures/book_cover}\\
    {
        \tiny
...
\begin{frame}{Data Breaches: Impact and Trends (2021--2025)\footnote{IBM / Ponemon Institute: The ``Cost of a Data Breach Report'' (2025 and 2024 editions).}}
    \begin{itemize}
        \item \textbf{Global Surge:} Data breaches reached record levels, with over \textbf{2.5 billion records} compromised in the last five years.
        \item \textbf{Financial Impact:}
        \begin{itemize}
            \item Average cost per lost record: \textbf{\$173}.
            \item High-risk sectors (e.g., Healthcare): Up to \textbf{\$429} per record.
        \end{itemize}
        \item \textbf{Primary Vulnerabilities:} Cloud repositories and misconfigured database servers account for \textbf{45\%} of all incidents.
        \item \textbf{Total Costs:} Includes forensic investigations, legal fees, and regulatory fines often exceeding \$250,000.
    \end{itemize}
\end{frame}

\begin{frame}{Data Breaches: Impact and Trends (2021--2025) (Cont.)}
    \centering
    \begin{tabular}{lll}
        \toprule
        \textbf{Metric} & \textbf{2005--2012 (Legacy)} & \textbf{2021--2025 (Updated)} \\
        \midrule
        Total Records & 544 Million & \textbf{$>$ 2.5 Billion} \\
        Cost per Record & \$90 -- \$305 & \textbf{\$173 -- \$429} \\
        Main Source & Database Servers & \textbf{Cloud \& Misconfigurations} \\
        \bottomrule
    \end{tabular}
\end{frame}

\section{Database Security Basics}
...
\begin{frame}{Open Source Tools: DBMS Enforcement}
    \begin{itemize}
        \item \textbf{PostgreSQL: \texttt{credcheck}}
        \begin{itemize}
            \item Enforces complexity (length, digits, special characters).
            \item Rejects passwords containing the username.
            \item Prevents plaintext password exposure in logs.
        \end{itemize}
        \item \textbf{MySQL/MariaDB: \texttt{validate\_password}}
        \begin{itemize}
            \item \textbf{Levels:} Low (length), Medium (complexity), Strong (dictionary).
            \item Dictionary checks prevent common/weak words.
            \item Provides a ``strength score'' for proposed credentials.
        \end{itemize}
    \end{itemize}
\end{frame}

\begin{frame}{Open Source Tools: Automation \& Identity}
    \begin{itemize}
        \item \textbf{OpenBao (Secrets Management\footnote{HashiCorp Vault fork.})}
        \begin{itemize}
            \item \textbf{Dynamic Secrets:} can generate secrets on-demand for some systems, such as Kubernetes or SQL databases.
            \item \textbf{Identity based access:} Use a unified ACL system to broker access to systems and secrets and merges identities across providers.
            \item Provides encryption as a service with centralized key management to simplify encrypting data.
        \end{itemize}
        \item \textbf{Authentik / Keycloak (Centralized Identity)}
        \begin{itemize}
            \item Centralizes ``Login Administration'' across multiple DBs.
            \item Enforces Multi-Factor Authentication (MFA) and SSO.
            \item Simplifies the ``Drop Login'' process when users leave.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\section{Granting and Revoking Authority}
...
\begin{frame}{Label-Based Access Control (LBAC)}
    ...
    \begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Homework]
        More in Lab 10...
    \end{tcolorbox}
\end{frame}
...
\section{Authorization Roles and Groups}
...
\section{Other Database Security Mechanisms}
...
\begin{frame}{SQL Injection Attacks}
    \begin{itemize}
        \item SQLi is a technique used by attackers to execute unauthorized SQL commands.
...
\begin{frame}{Recent SQLi Incidents (2023--2026)}
    \begin{itemize}
        \item \textbf{\href{https://community.progress.com/s/article/MOVEit-Transfer-Critical-Vulnerability-31May2023}{MOVEit Transfer (2023)}:} CVE\footnote{Common Vulnerabilities and Exposures}-2023-34362 allowed the exfiltration of data from \textbf{2,700+ organizations} affecting \textbf{93.3 million individuals}, including global firms and government agencies.
        \item \textbf{\href{https://nvd.nist.gov/vuln/detail/CVE-2025-1871}{101news CMS Vulnerabilities (2025)}:} CVE-2025-1871 allowed unauthenticated remote command execution via category parameters.
        \item \textbf{\href{https://www.postgresql.org/support/security/CVE-2025-1094/}{Driver Risks (2025)}:} CVE-2025-1094 in PostgreSQL's \texttt{libpq} highlights risks in foundational database libraries.
    \end{itemize}
\end{frame}

\begin{frame}{Case Study: Cybersecurity in Colombia}
    \begin{itemize}
        \item \textbf{2023 State Attacks:} \textbf{32 state entities} (e.g., MinSalud, SIC) faced major breaches compromising citizen data.
        \item \textbf{Targeted Region:} In 2024, Colombia suffered \textbf{36 billion} cyberattack attempts, ranking 2nd in Latin America.
        \item \textbf{Improved Defense:} Reports for 2025 show a \textbf{48\% reduction} in successful incidents due to ColCERT strategies.
        \item \textbf{Sector Focus:} SQLi remains a top threat for the Colombian financial and healthcare sectors in 2026.
    \end{itemize}
\end{frame}

\begin{frame}{Modern Trends and Challenges}
    \begin{itemize}
        \item \textbf{AI-Assisted Coding Risks:} AI tools often generate code lacking proper input validation, re-introducing SQLi flaws.
        \item \textbf{Technical Debt:} Mission-critical legacy systems (10--20 years old) often use outdated, vulnerable query patterns.
        \item \textbf{Persistence:} As of 2025, \textbf{38\%} of application weaknesses are still attributed to SQLi or Cross-Site Scripting (XSS).
        \item \textbf{DBA Role:} Moving from manual sanitization to automated policy enforcement using modern middleware.
    \end{itemize}
\end{frame}
...
\begin{frame}{Top 5 Fundamental Takeaways: Database Security}
    \begin{itemize}
        \item[5] \textbf{Secure the ``Four Pillars'':} Authentication, Authorization, Encryption, and Auditing. The DBMS centralizes control over all security resources. \pause

        \item[4] \textbf{Enforce Password Hygiene:} Use tools like \texttt{credcheck} to automate complexity. Block weak archetypes and the use of personal information. \pause

        \item[3] \textbf{Least Privilege via Roles:} Use Roles and Groups to simplify administration. Beware of cascading effects from the \texttt{WITH GRANT OPTION}. \pause

        \item[2] \textbf{Granular Access Control:} Implement LBAC and Views for row/column-level security. Use Stored Procedures to enforce controlled modification logic. \pause

        \item[1] \textbf{Proactive Defense:} Prevent SQLi using prepared statements and input validation. Perform regular reviews and auditing to detect evolving threats.
