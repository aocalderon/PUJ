...
\begin{frame}{Indexes (Cont.)}
  \centering
  \scriptsize
  \begin{tabularx}{\textwidth}{@{} l >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X @{}}
    \toprule
      \textbf{Type} & \textbf{Pros} & \textbf{Cons} & \textbf{Best Example} \\
    \midrule
      \textbf{B-Tree} & Versatile; supports sorting, ranges, and unique constraints. & Can grow very large; high overhead on frequent updates. & Primary Keys: Searching for a specific \texttt{user\_id} or price ranges. \\
      \textbf{Hash} & Faster than B-Tree for exact equality (\texttt{=}) matches. & No support for ranges (\texttt{>}, \texttt{<}) or sorting. & Unique Tokens: Looking up a specific \texttt{session\_id} or API key. \\
      \textbf{GiST} & Highly flexible; handles complex geometry, ranges, and KNN. & Slower to build and search than GIN or B-Tree. & Geography: Finding coffee shops within 1km of a coordinate. \\
      \textbf{GIN} & Best for multi-value data; lightning-fast read performance. & Heavy ``write tax''; slow to update or insert new rows. & JSONB/Arrays: Finding products with the tag \texttt{'organic'}. \\
      \textbf{BRIN} & Extremely small footprint; minimal overhead on massive tables. & Only effective if data is physically sorted on disk. & Time-Series: Scanning logs from a billion-row table by timestamp. \\
    \bottomrule
  \end{tabularx}
\end{frame}

\begin{frame}[fragile]{Indexes  (Cont.)}
    \begin{minted}
...
\begin{frame}{Index Creation Notes}
  \begin{itemize}
    \item \textbf{CREATE} and \textbf{DROP} indexes are not online activities by default.  The table is locked for modifications during these operations.
    \item Read operations can still be performed during index creation/modification if the \texttt{CONCURRENTLY} keyword is used.
    \item Multi-column indexes can be created to improve performance on queries involving multiple columns.
    \item When creating multi-column indexes, ensure the column order matches the order used in the query’s \texttt{WHERE} clause.
  \end{itemize}
\end{frame}
...
TABLESPACE new_tablespace: Reindexing could be done in a new tablespace
...
\begin{frame}{TDT5FTOTC}
    \centering
    \includegraphics[width=\textwidth]{figures/tim.png}
\end{frame}
