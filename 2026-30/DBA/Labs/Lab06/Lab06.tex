...
These topics are covered in Chapter 13 of \textit{PostgreSQL for Jobseekers} (2023), by Sonia Valeja and David Gonzales, starting on page~243. You are expected to {\Large \textbf{handwrite}} a concise summary of these points, scan the handwritten work, and append it to the report. Only one summary is required for submission.

\section{Data Generation and Performance Evaluation}
In this section, we will first generate a dataset to simulate the activity of a forum with accounts, threads, and posts. This synthetic data will provide the foundation for executing a set of predefined queries. Once the data is in place, we will measure the performance of these queries both before and after applying indexes.
By comparing execution times using the \texttt{EXPLAIN ANALYZE} command in PostgreSQL, we will gain practical insights into how indexing strategies improve query efficiency. This integrated approach will allow us to connect the process of data generation directly with its impact on query performance.
...
\subsection{Measuring Performance Without and With Indexes}
...
Use your preferred statistical software (e.g., R, Octave, Python, Scilab, Gnuplot, Grace, Veusz, etc.) to generate the graph.
...
We expect you to submit your report before the lab on \textbf{14 days} from now.  Please, have a look at Brightspace\texttrademark~for the current deadline.
