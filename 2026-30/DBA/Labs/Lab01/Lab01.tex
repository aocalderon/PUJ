...
The primary objective of this lab is to explore automated data dictionary generation using \href{https://schemaspy.org/}{SchemaSpy}. As a multiplatform DBA tool, SchemaSpy connects to various DBMS platforms via the command line to extract key metadata on tables, attributes, and relationships. In this lab, we will work through a foundational guide before exploring alternative metadata tools as part of your independent work.

\section{SchemaSpy Tutorial}

\begin{enumerate}
    \item First, we will download SchemaSpy v6.2.4 from \href{https://github.com/schemaspy/schemaspy/releases/tag/v6.2.4}{here}. SchemaSpy is a Java application, so ensure that you have a stable JRE installed on your system. If Java is not present in Ubuntu or if you want to update or change the current installation, follow the steps in \href{https://www.linuxcloudvps.com/blog/install-java-lts-on-ubuntu-24-04/}{this} tutorial.
    ...
    \begin{boxI}
        \textbf{Note}: \textit{You may need to adjust the full path and name of the file as necessary}.
    \end{boxI}
    ...
    At the time of writing, the current version is 42.7.7, so you should download a file named \texttt{postgresql-42.7.7.jar}.  Feel free to use an updated version if you prefer. Please place the driver in the same location as the SchemaSpy application.

    \item Next, lets create a folder called \texttt{dd} in the same directory as \texttt{schemaspy} and the JDBC driver to store the results.
...
We expect two things from you and your team as autonomous work:
...
We expect you to submit a well-structured report in PDF format, along with a dump file (a plain text with .sql extension) of the fixed MRD, packaged in a ZIP file, by \textbf{August 17, 2026 before 2:00PM}, via the link that will be provided on BrightSpace\texttrademark.
