...
\begin{frame}{Extension to Other Models}
    \begin{itemize}
        \item Although we have presented best subset selection here for least squares regression, the same ideas apply to other types of models, such as logistic regression.
        \item The \textit{deviance}\textbf{} --negative two times the maximized log-likelihood-- plays the role of RSS for a broader class of models.
    \end{itemize}
\end{frame}
...
\begin{frame}{Stepwise Selection}
    \begin{itemize}
        \item For computational reasons, best subset selection cannot be applied with very large $p$. \textbf{Why not?} \pause
        $2^p \implies 2^{10} \approx 1000$ but $2^{40} \approx 1.1 \times 10^{12}$

        \item Best subset selection may also suffer from statistical problems when $p$ is large: larger the search space, the higher the chance of finding models that look good on the training data, even though they might not have any predictive power on future data.\pause

        \item Thus an enormous search space can lead to \textbf{overfitting} and high variance of the coefficient estimates (we could find an extremely good model in the training data by chance, some combination of noise features will align with the training response simply by random chance).\pause

        \item For both of these reasons, \textbf{stepwise} methods, which explore a far more restricted set of models, are attractive alternatives to best subset selection.
    \end{itemize}
\end{frame}
...
\begin{frame}{Forward Stepwise Selection}
    \begin{itemize}
        \item Best subset selection is computationally intractable for large $p$.
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}[(a)]
            ...
            \item Choose the \textbf{best} among these $p - k$ models (smallest RSS / highest $R^2$), and call it $\mathcal{M}_{k+1}$.
        \end{enumerate}
        \item Select the overall best model among $\mathcal{M}_0, \dots, \mathcal{M}_p$ via CV, $C_p$, AIC, BIC, or adjusted $R^2$.
    \end{enumerate}
    \end{block}
    \begin{itemize}
        \item Fits only $1 + \sum_{k=0}^{p-1}(p-k) = 1 + \frac{p(p+1)}{2} \approx p^2$ models (for $p=20$: 211 vs. 1,048,576).
    \end{itemize}
\end{frame}
...
\begin{frame}{Backward Stepwise Selection}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}[(a)]
            ...
            \item Choose the \textbf{best} among these $k$ models (smallest RSS / highest $R^2$), and call it $\mathcal{M}_{k-1}$.
        \end{enumerate}
        \item Select the single best model among $\mathcal{M}_0, \dots, \mathcal{M}_p$ using CV, $C_p$, AIC, BIC, or adjusted $R^2$.
    \end{enumerate}
    \end{block}
    \begin{itemize}
        \item Like forward stepwise, considers only $1 + \frac{p(p+1)}{2} \approx p^2$ models.
...
\begin{frame}{Choosing the Optimal Model: Adjusted Criteria}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Mallows' $C_p$ and Akaike Information Criteria ($AIC$)}
                \[
                ...
                \begin{itemize}
                    \small
                    \item where $L$ is the \textit{maximized value of the likelihood} fucntion.
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Bayesian Information Criteria ($BIC$)  and Adjusted $R^2$}
                \[
                    BIC = \frac{1}{n}\left(\text{RSS} + \log(n)d\hat{\sigma}^2\right)
                \]
                \[
                    Adjusted \text{ } R^2 = 1 - \frac{\text{RSS}/(n-d-1)}{\text{TSS}/(n-1)}
                \]
                \begin{itemize}
                    \small
...
\begin{frame}{Cross-validation}
    \centering
    \includegraphics[width=0.9\textwidth]{figures/cross}
\end{frame}
...
\begin{frame}{Model Selection: Adjusted Criteria vs. Validation/CV}
    ...
    \begin{itemize}
        \footnotesize
        \item Plots of $C_p$, $BIC$, and Adjusted $R^2$ for \texttt{Credit} data models.
...
\begin{frame}{Ridge Regression}
    \begin{itemize}
        ...
        \item Uses the squared $\ell_2$ penalty: $\|\beta\|_2 = \sqrt{\sum_{j=1}^p \beta_j^2}$.
    \end{itemize}
\end{frame}

\begin{frame}{Ridge Regression: Coefficient Profiles on Credit Data}
    \centering
    \includegraphics[width=0.9\textwidth]{figures/6_4}
    \begin{itemize}
        \footnotesize
...
\begin{frame}{Comparing Lasso and Ridge Regression}
    ...
    \begin{itemize}
        \footnotesize
        \item \textbf{Left}: Plots of squared bias (black), variance (green), and test MSE (purple) for the lasso on simulated data set ($n=50$ and $p=45$).
...
\begin{frame}{Comparing Lasso and Ridge Regression}
    ...
    \begin{itemize}
        \footnotesize
        \item \textbf{Left}: Plots of squared bias (black), variance (green), and test \textit{MSE} (purple) for the lasso. The simulated data is similar to that in previous slide, except that now only two predictors are related to the response.
