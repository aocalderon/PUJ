...
\begin{frame}{Classify to the Highest Density}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \node at (-11.25, 2.00) (y) {$\pi_k f_k(x)$};
...
\begin{frame}{Linear Discriminant Analysis when $p = 1$}
  ...
  \]
  \pause
  Happily, there are simplifications and cancellations\footnote{Shared variance across all classes is the foundational assumption that defines LDA in order to cancel quadratic terms.}.
\end{frame}
...
\begin{frame}{Linear Discriminant Analysis when $p > 1$}
  \centering
  \includegraphics[width=0.85\textwidth]{figures/p27} \\
