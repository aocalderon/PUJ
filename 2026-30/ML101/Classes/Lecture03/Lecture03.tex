...
\begin{frame}{Estimation of the Parameters by Least Squares}
  \begin{itemize}
    \item Let $\hat{y}_i = \hat{\beta}_0 + \hat{\beta}_1 x_i$ be the prediction for $Y$ based on the $i$-th value of $X$. Then $e_i = y_i - \hat{y}_i$ represents the $i$-th residual.
    \item We define the residual sum of squares (RSS) as:
...
\begin{frame}{Hypothesis Testing -- continued}
  \begin{itemize}
    \item To test the null hypothesis, we compute a \textbf{$t$-statistic}, given by:
    \[ t = \frac{\hat{\beta}_1 - 0}{SE(\hat{\beta}_1)} \]
    \item This will have a $t$-distribution with $n-2$ degrees of freedom, assuming $\beta_1 = 0$.
    \item Using statistical software, it is easy to compute the probability of observing any value equal to $|t|$ or larger. We call this probability the \textbf{$p$-value}.
  \end{itemize}
\end{frame}
...
\subsection{Estimating the Regression Coeﬀicients}
