...
\title{Databases \\ Lab 05: A `gentle' Introduction Guide for the First Partial.}
...
Welcome to Lab 05. This session is specifically designed to serve as your primary study resource for the \textbf{first partial exam}. In this lab, we take a hands-on approach to data analysis using SQL within in the context of European football leagues.

By working with a rich dataset that spans multiple seasons and thousands of players and matches, you will gain the practical experience needed to design SQL queries that extract meaningful insights from relational data. Consider this lab a "dry run" for the types of logical challenges you will face in your upcoming evaluation.

\section{Exam Preparation Objectives}
To ensure you are fully prepared for the partial, this lab focuses on three core areas of intermediate SQL mastery:

\begin{itemize}
    \item \textbf{SQL Ordering \& Reasoning:} Practice reordering and interpreting complex SQL lines involving SQL commands such as \texttt{UNION}, \texttt{INTERSECT}, and \texttt{CASE} statements.
    \item \textbf{Common Table Expressions (CTEs):} Master the use of \texttt{WITH} clauses to create temporal relations, which are essential for comparing individual performance against global averages.
    \item \textbf{Data Synthesis:} Learn to join multiple tables (Players, Attributes, Matches, and Leagues) to rank elite players and analyze team performance.
\end{itemize}
...
\begin{figure}
 \centering
 \includegraphics[width=0.9\textwidth]{figures/Euroleagues_erd.jpg}
