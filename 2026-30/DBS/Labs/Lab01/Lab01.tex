...
\begin{itemize}
    \item Download the \href{https://www.cs.ucr.edu/~acald013/public/Javeriana/DBS/applications.db}{\texttt{applications.db}} file, which contains a sample database about students,  colleges and their appplication process.
...
Your task is to connect to the \texttt{beer.db} database in the same way as with \texttt{applications.db} and follow the guide by running all the examples provided. Pay particular attention to the last three group of operations:

\begin{itemize}
  \item Set union, difference, and intersection,
  \item Rename, and
  \item Aggregation and Grouping.
\end{itemize}
...
Finally, you will propose your own queries for the \texttt{pizza.db} database (one per member of your team). Based on the queries above, you will create new queries from scratch, write their corresponding relational algebra expressions, and provide the \texttt{radb} code to answer them.

We expect you to submit a well-structured report in \textbf{\large PDF format}, containing the requested information outlined in Sections \ref{sec:beer} and \ref{sec:pizza}. The submission deadline is 14 days from now (before lab) via the corresponding assigment in BrightSpace\texttrademark.
