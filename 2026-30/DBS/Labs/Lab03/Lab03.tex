...
The objective of this lab is to reinforce database querying skills by executing and analyzing SQL queries that extract meaningful insights from the dataset. Students will work in groups by their own to execute a series of predefined queries, capture their results, and document their findings. Additionally, students will be required to propose original queries using different types of \texttt{JOIN} operations.
...
Be sure to capture a selfie once you complete the assigned lessons, so you can include it later in your report. Whenever possible, make sure your username is visible in your selfie. This portion of the lab must be completed individually, meaning each group member is required to demonstrate their own work.
...
Now that we are in the PostgreSQL prompt, we will use the \texttt{\textbackslash i} command to load the files into our database. \textbf{Remember you will need to update the file paths accordingly}.
