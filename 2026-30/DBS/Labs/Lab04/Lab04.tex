...
\title{Databases \\ Lab 04: A `gentle' Introduction to SQL Views.}
...
\section{W3Schools Interactive SQL Tutorial} \label{sec:w3s}
...
\begin{boxH}
It is just an informative section.  You do not need to run the SQL code.  The main goal is to get familiar with the syntax and commands to create and manage Views.
\end{boxH}
...
SQL Views provide a powerful way to structure queries, simplify access to data, and enforce security rules. Understanding when and how to use them can significantly improve database design and maintainability.  No submission is required for this section, but the material will be applied in later parts of the lab.

\section{SQL Script: Working with Views} \label{sec:views1}

\subsection{Step 1: Create Sample Tables and Insert Data}

To practice these concepts, run the following SQL script in PostgreSQL (psql or pgAdmin) to generate the necessary simulation tables:
...
You may now execute the following code snippets in the rest of section \ref{sec:views1} and section \ref{sec:views2}  to view the results.
...
\section{Advanced SQL Views Tutorial} \label{sec:views2}
...
Do not execute the following SQL command. It is intended solely to illustrate the use of the \texttt{GRANT} command for granting limited permissions to a hypothetical \texttt{public\_user}.
...
\section{Hands-On Exercises} \label{sec:exercises}
...
\subsubsection*{Task:}
Create a view called `PublicEmployees' that \textbf{hides salary information} and only shows EmployeeID, FirstName, LastName, and DepartmentName.  Then, grant access to this view for a user named \texttt{readonly\_user}\footnote{The existence of the user in your environtment is not required; for this exercise the purpose is solely to provide the corresponding SQL syntax for the \texttt{GRANT} command.}.
...
For each query, you must provide both the corresponding \texttt{SQL} code and the query result (either copy-pasted or as a screenshot).

\section{Autonomous Work}

Prepare a \textbf{well-structured} report in PDF format incorporating all required information from Sections \ref{sec:w3s} and \ref{sec:exercises}. Additionally, provide a plain text file with a \texttt{.sql} extension containing the \texttt{SQL} code for your solutions.

Consolidate both the PDF report and the \texttt{.sql} file into a single ZIP archive for submission via Brightspace\texttrademark. The submission must be completed by \textbf{21 days} from now before the start of the lab session on that.
