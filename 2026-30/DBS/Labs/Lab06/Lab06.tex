...
In the video, several types of index implementations are presented --just select \textbf{the most effective one} for your analysis and plotting. Use your preferred statistical software (e.g., R, Python, Octave, Gnuplot, etc.) to generate the plots.  Please, avoid use spreadsheets for plotting, it is horrible.  \textbf{Spreadsheets are for accounting, not for plotting.}
...
We expect you to submit your report through Brightspace\texttrademark{} no later than \textbf{14 days} from now (before the lab session on that day).
