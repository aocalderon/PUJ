...
We will watch a YouTube playlist from the \href{https://www.youtube.com/@w3schools}{W3Schools.com} channel covering introductory SQL. You can access the playlist \href{https://youtube.com/playlist?list=PLP9IO4UYNF0UQkBXlTMSw0CYsxv-GDkkI&si=qel9VClPHmerhGlz}{here}. If you prefer, you can enable Spanish subtitles via the Settings button. The playlist consists of 11 short videos. Simply watch them, \textit{\textbf{there is no specific assignment or submission required for this section}}.
...
Once you have completed all the lessons, take a screenshot similar to Figure~\ref{fig:panel}. Make sure your profile picture and your progress (green line in the left panel) are clearly visible.  If you cannot set a profile picture you must to take a picture of you next to your work. Each team member must provide his/her own screenshot.
...
Download both files and place them in the same folder. We will assume that you already have a functional PostgreSQL instance running on your machine. You can follow this \href{https://youtu.be/YZn_LiGjJnc?si=3igbviSXa2Z4xrUY}{YouTube video} from Roel Code Channel\footnote{Alex Roel, 2026. \url{https://www.youtube.com/@roelcode}} to follow a step  by step tutorial to install \href{https://www.postgresql.org/}{PostgreSQL} and \href{https://www.pgadmin.org/}{PgAdmin} in your virtual machine.  You should be able to access a PostgreSQL console (\texttt{psql} prompt) similar to Figure~\ref{fig:psql}. \\
...
Once in the PostgreSQL Console, create a database named \texttt{university}:
...
You are now in the PostgreSQL prompt. From here, use the \texttt{\textbackslash i} command to load the schema and data for the database. \textbf{\textit{Remember to update the file paths to match the location where you saved the downloaded files.}}
...
You are now ready to explore the SQL queries we have already covered in class. If you encounter any issues, watch this \href{https://youtu.be/2hco9E1R6K8}{video}, which provides a detailed walkthrough of how to load the schema and data for the \texttt{university} database. \textit{Please, do not pay attention to what I said in the first 40 seconds of the video, that was related to the previous classes.}
...
\begin{enumerate}
  ...
  \item Propose your own query and solve it using SQL.  One per each member of the team.
\end{enumerate}

For each query, you must provide its corresponding \textit{relational algebra} expression, the \texttt{SQL} code, and the query result (either a copy–paste of the console or a screenshot).

We expect a well-structured report in \textbf{PDF} format containing the requested information from the previous sections. Additionally, include a plain-text file with extension \texttt{.sql} that contains all the statements from Section~\ref{sec:work} (use comments to point the exercise number and the corresponding statement). Package both files (\texttt{.pdf} and \texttt{.sql}) into a single \textbf{ZIP} file and submit it via Brightspace\texttrademark. The submission deadline is 14 days from now, please, have a look at the deadline in the platform.
